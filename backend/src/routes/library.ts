...
libraryRouter.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occured while getting all libraries: ${err}` });
  }
});
...
libraryRouter.post('/add-library', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occured while adding a library: ${err}` });
  }
});
...
libraryRouter.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Error occured while deleting a library ${err}` });
    }
  },
);
...
libraryRouter.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Error occured while updating a library ${err}` });
    }
  },
);
